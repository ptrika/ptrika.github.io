...
function(window, factory) {
    ...
    var matchesMethod = function() {
        ...
        for (var prefixes = ["webkit", "moz", "ms", "o"], i = 0; i < prefixes.length; i++) {
            var prefix, method = prefixes[i] + "MatchesSelector";
...
function(window, factory) {
    ...
    utils.extend(proto, EvEmitter.prototype), proto.option = function(opts) {
        ...
        for (var itemElems = this._filterFindItemElements(elems), Item = this.constructor.Item, items = [], i = 0; i < itemElems.length; i++) {
            var elem, item = new Item(itemElems[i], this);
            items.push(item)
        }
        return items
        ...
        if (null != stagger) return this.stagger = getMilliseconds(stagger), this.stagger;
        this.stagger = 0
...
function(window, factory) {
    ...
    return proto._resetLayout = function() {
        ...
        var isFitWidth, container = this._getOption("fitWidth") ? this.element.parentNode : this.element,
            size = getSize(container);
        this.containerWidth = size && size.innerWidth
    }, proto._getItemLayoutPosition = function(item) {
        item.getSize();
        var remainder = item.size.outerWidth % this.columnWidth,
            mathMethod, colSpan = Math[remainder && remainder < 1 ? "round" : "ceil"](item.size.outerWidth / this.columnWidth);
        colSpan = Math.min(colSpan, this.cols);
        for (var colPosMethod, colPosition = this[this.options.horizontalOrder ? "_getHorizontalColPosition" : "_getTopColPosition"](colSpan, item), position = {
        ...
        var stampSize = getSize(stamp),
            ...
            isOriginLeft, firstX = this._getOption("originLeft") ? offset.left : offset.right,
        ...
        for (var isOriginTop, stampMaxY = (this._getOption("originTop") ? offset.top : offset.bottom) + stampSize.outerHeight, i = firstCol; i <= lastCol; i++) this.colYs[i] = Math.max(stampMaxY, this.colYs[i])
    ...
    var mungeSorter = function() {
        function mungeSorter(sorter) {
            ...
            var args = trim(sorter).split(" "),
                ...
                attr, getValue = getValueGetter(attrMatch && attrMatch[1], query),
        ...
        function getValueGetter(attr, query) {
            return attr ? function getAttribute(elem) {
    ...
    function getItemSorter(sortBys, sortAsc) {
        return function sorter(itemA, itemB) {
            for (var i = 0; i < sortBys.length; i++) {
                ...
                if (a > b || a < b) return (a > b ? 1 : -1) * ((void 0 !== sortAsc[sortBy] ? sortAsc[sortBy] : sortAsc) ? 1 : -1)
            }
            return 0
        }
    }
